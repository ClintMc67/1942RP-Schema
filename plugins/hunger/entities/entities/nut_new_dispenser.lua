...
ENT.Type = "anim"
ENT.PrintName = "Ration New Dispenser"
ENT.Author = "LiGyH"
ENT.Category = "HL2 RP"
ENT.Spawnable = true
ENT.AdminOnly = true
	...
	function ENT:createRation()
		...
		timer.Simple(1.2, function()
			if (IsValid(self) and IsValid(entity)) then
				entity:Remove()
				nut.item.spawn("new_ration", entity:GetPos(), nil, entity:GetAngles())
			end
		end)
	end
	...
	function ENT:Use(activator)
		...
		if (activator:Team() == FACTION_CITIZEN) then
			...
			timer.Simple(1, function()
				...
				elseif (found and amount == 0 and self:GetChar() <= 0) then
		...
		elseif (activator:isCombine() or activator:Team() == FACTION_CWU) then
			local item = activator:getChar():getInv():hasItem("ration")
			if item then
			for k, v in pairs(activator:getChar():getInv():getItems()) do
				if (v.uniqueID == "ration") then
					v:remove()
				end
			end
			self:SetChar(self:GetChar()+1)
