...
function ENT:SpawnFunction(client, trace)
	local entity = ents.Create("nut_ration_creator")
	...
	function ENT:Use(activator)
		...
		if (activator:Team() == FACTION_CITIZEN) then
			...
			timer.Simple(1, function()
				...
				local item = activator:getChar():getInv():hasItem("gel")
				local item2 = activator:getChar():getInv():hasItem("water")
