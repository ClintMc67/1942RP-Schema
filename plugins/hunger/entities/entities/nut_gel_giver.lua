...
function ENT:SpawnFunction(client, trace)
	local entity = ents.Create("nut_gel_giver")
	...
	function ENT:createRation()
		...
		timer.Simple(1.2, function()
			if (IsValid(self) and IsValid(entity)) then
				entity:Remove()
				nut.item.spawn("gel", entity:GetPos(), nil, entity:GetAngles())
			end
		end)
	end
	...
	function ENT:Use(activator)
		...
		if (activator:Team() == FACTION_CITIZEN) then
			...
			timer.Simple(1, function()
				...
				local item = activator:getChar():getInv():hasItem("gel_empty")
